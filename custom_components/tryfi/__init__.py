...
async def async_remove_config_entry_device(
    ...
    if device_entry.name and "Base Station" in device_entry.name:
        _LOGGER.info(
            "Allowing removal of duplicate base station device: %s", device_entry.name
        )
        return True
    ...
    if device_entry.model == "TryFi Base":
        _LOGGER.info(
            "Allowing removal of old format base device: %s", device_entry.name
        )
        return True
        ...
        for identifier in device_entry.identifiers:
            if identifier[0] == DOMAIN:
                ...
                if device_id.startswith("base_"):
                    _LOGGER.info(
                        "Allowing removal of old format base device with base_ prefix: %s",
                        device_id,
                    )
                    return True
                # If the device is no longer in the API, allow removal
                if (
                    device_id not in current_pet_ids
                    and device_id not in current_base_ids
                ):
    ...
    _LOGGER.info(
        "Device %s is still active in the API, removal not allowed", device_entry.name
    )
    return False
        ...
        for entry_id, coordinator in hass.data[DOMAIN].items():
            if isinstance(coordinator, TryFiDataUpdateCoordinator):
                ...
                for pet in coordinator.data.pets:
                    if (
                        entity_id
                        == f"light.{pet.name.lower().replace(' ', '_')}_collar_light"
                    ):
                        await hass.async_add_executor_job(
                            pet.setLedColorCode, coordinator.data.session, color_code
                        )
            ...
            if isinstance(coordinator, TryFiDataUpdateCoordinator):
                for pet in coordinator.data.pets:
                    if (
                        ...
                        await hass.async_add_executor_job(
                            pet.turnOnOffLed, coordinator.data.session, True
                        )
                        await coordinator.async_request_refresh()
                        return
            ...
            if isinstance(coordinator, TryFiDataUpdateCoordinator):
                for pet in coordinator.data.pets:
                    if (
                        ...
                        await hass.async_add_executor_job(
                            pet.turnOnOffLed, coordinator.data.session, False
                        )
                        await coordinator.async_request_refresh()
                        return
        ...
        for entry_id, coordinator in hass.data[DOMAIN].items():
            if isinstance(coordinator, TryFiDataUpdateCoordinator):
                for pet in coordinator.data.pets:
                    if (
                        entity_id
                        == f"select.{pet.name.lower().replace(' ', '_')}_lost_mode"
                    ):
                        await hass.async_add_executor_job(
                            pet.setLostDogMode, coordinator.data.session, is_lost
                        )
                        await coordinator.async_request_refresh()
                        return
