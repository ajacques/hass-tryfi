...
QUERY_PET_ACTIVE_DETAILS = (
    'query {  pet (id: "'
    + VAR_PET_ID
    + '") { ongoingActivity { __typename ...OngoingActivityDetails } dailyStepStat: currentActivitySummary (period: DAILY) { ...ActivitySummaryDetails } weeklyStepStat: currentActivitySummary (period: WEEKLY) { ...ActivitySummaryDetails } monthlyStepStat: currentActivitySummary (period: MONTHLY) { ...ActivitySummaryDetails } device { __typename moduleId info operationParams {    __typename    ...OperationParamsDetails  }  nextLocationUpdateExpectedBy  lastConnectionState {    __typename    ...ConnectionStateDetails  }  ledColor {    __typename    ...LedColorDetails }} dailySleepStat: restSummaryFeed(cursor: null, period: DAILY, limit: 1) {      __typename      restSummaries {        __typename        ...RestSummaryDetails }} monthlySleepStat: restSummaryFeed(cursor: null, period: MONTHLY, limit: 1) {      __typename      restSummaries {        __typename        ...RestSummaryDetails }} }}'
)
QUERY_PET_ACTIVITY = (
    'query {  pet (id: "'
    + VAR_PET_ID
    + '") {       dailyStat: currentActivitySummary (period: DAILY) {      ...ActivitySummaryDetails    }    weeklyStat: currentActivitySummary (period: WEEKLY) {      ...ActivitySummaryDetails    }    monthlyStat: currentActivitySummary (period: MONTHLY) {      ...ActivitySummaryDetails    }  }}'
)
QUERY_PET_CURRENT_LOCATION = (
    'query {  pet (id: "'
    + VAR_PET_ID
    + '") {    ongoingActivity {      __typename      ...OngoingActivityDetails    }  }}'
)
...
def setLedColor(session: requests.Session, deviceId: str, ledColorCode):
    ...
    qVariables = {"moduleId": deviceId, "ledColorCode": ledColorCode}
...
def turnOnOffLed(session: requests.Session, moduleId, ledEnabled: bool):
    ...
    qVariables = {"input": {"moduleId": moduleId, "ledEnabled": ledEnabled}}
    response = mutation(session, qString, qVariables)
...
def query(session: requests.Session, qString):
    ...
    try:
        ...
        raise RemoteApiError(
            f"Invalid JSON response from API: {e}. First few bytes: '{resp.text[:10]}'"
        ) from e

    if "errors" in json_object:
        error_msg = ",".join(
            map(
                lambda x: x.get("message", "Unknown GraphQL error"),
                json_object["errors"],
            )
        )
        if any(
            auth_err in error_msg.lower()
            for auth_err in [
                "unauthorized",
                "unauthenticated",
                "authentication",
...
def _execute(
    ...
    if method == "GET":
        ...
        raise TryFiError(
            f"Method Passed was invalid: {method}. Only GET and POST are supported"
        )
