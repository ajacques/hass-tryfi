...
def test_update_with_auth_error_triggers_reauth():
    ...
    with patch("custom_components.tryfi.pytryfi.PyTryFi.login") as mock_login:
        with patch(
            "custom_components.tryfi.pytryfi.PyTryFi.updateBases"
        ) as mock_update_bases:
            with patch(
                "custom_components.tryfi.pytryfi.PyTryFi.updatePets"
            ) as mock_update_pets:
...
def test_query_auth_error_detection():
    ...
    response.json.return_value = {"errors": [{"message": "User is not authenticated"}]}
